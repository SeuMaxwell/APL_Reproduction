...
import numpy as np
import scipy as sp
from lum_lib.utils.scipy_wrappers import wrapped_GridInterpolator
...
class Fields(object):
    ...
    #
    #     ax.pcolormesh(xx*1e6, yy*1e6, np.abs(Ex**2+Ey**2+Ez**2), shading='auto', cmap=plt.get_cmap(cmap))
    ...
    def plot(self, ax, title, cmap):
        ...
        # 关键修复: 只使用一个波长值(中间的波长)
        # 如果 self.wl 是数组，取中间元素，否则直接使用
        if hasattr(self.wl, '__len__') and len(self.wl) > 0:
            mid_index = len(self.wl) // 2  # 整除取中间索引
            single_wl = self.wl[mid_index]
        else:
            single_wl = self.wl
        print(f"Using single wavelength: {single_wl}")
        # 如果 self.wl 是数组，取第一个元素，否则直接使用
        # single_wl = self.wl[0] if hasattr(self.wl, '__len__') and len(self.wl) > 0 else self.wl
        # print(f"Using single wavelength: {single_wl}")

        # 重要修改: 展平网格进行循环，然后重塑结果
        xx_flat = xx.flatten()
        yy_flat = yy.flatten()
        ...
        for i in range(len(xx_flat)):
            field = self.getfield(xx_flat[i], yy_flat[i], z, single_wl)
            E_fields_flat.append(field)

        # 从场数据中提取分量
        Ex = np.array([E[0] for E in E_fields_flat]).reshape(xx.shape)
        Ey = np.array([E[1] for E in E_fields_flat]).reshape(xx.shape)
        Ez = np.array([E[2] for E in E_fields_flat]).reshape(xx.shape)

        # 绘制场强度
        mesh = ax.pcolormesh(xx * 1e6, yy * 1e6, np.abs(Ex ** 2 + Ey ** 2 + Ez ** 2), shading='auto',
...
class FieldsNoInterp(Fields):
    ...
    def make_field_interpolation_object_nointerp(self,F):                         #线性插值
